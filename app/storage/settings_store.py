...
FIELDS: list[Field] = [
    ...
    Field("adx_min", "float", "Минимальная сила тренда (ADX)", "strategy",
          hint="ADX измеряет, насколько уверенно цена идёт в одну сторону. "
    ...
    Field("ema_fast", "int", "Быстрая EMA", "strategy",
          hint="Период короткой скользящей средней. Её пересечение "
    ...
    Field("adx_period", "int", "Период ADX", "strategy",
          hint="За сколько свечей считать силу тренда. 14 — классическое "
               "значение, менять обычно не нужно.",
          minimum=2, maximum=100, advanced=True),
    Field("rsi_period", "int", "Период RSI", "strategy",
          hint="За сколько свечей считать перегретость движения. "
    ...
    Field("atr_sl_mult", "float", "Ширина стопа", "risk",
          hint="Насколько далеко ставить страховочный уровень. Считается от "
    ...
    Field("atr_period", "int", "Период ATR", "risk",
          hint="За сколько свечей измерять дёрганость рынка для расчёта "
